...
async function main() {
  ...
  const url = 'https://de1.api.radio-browser.info/json/stations/topclick/30';
  ...
  for (let i = 0; i < stations.length; i++) {
    const s = stations[i];
    const tagsArr = s.tags ? s.tags.split(',') : [];
    const categoryName = tagsArr[0] || 'General';
    ...
    await prisma.station.upsert({
      ...
      create: {
        ...
        slug: s.stationuuid,
        name: s.name,
        description: `Language: ${s.language}, Country: ${s.country}. Tags: ${s.tags}`,
