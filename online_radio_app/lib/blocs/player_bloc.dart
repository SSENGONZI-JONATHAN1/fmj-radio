...
class PlayerBloc extends Bloc<PlayerEvent, PlayerState> {
  ...
  Timer? _sleepTimer;
  // Keep the latest now-playing info so we can apply it when playback starts
  NowPlayingInfo _lastNowPlaying = NowPlayingInfo.empty();
  ...
  PlayerBloc({required AudioPlayerService audioPlayerService})
    ...
    // Listen to now playing updates and cache the latest info
    ...
    // Listen to playing state and transition appropriately. Allow transition
    // from PlayerLoading -> PlayerPlaying when the audio service reports
    ...
    _isPlayingSubscription = _audioPlayerService.isPlayingStream.listen((isPlaying) {
      if (isPlaying) {
        ...
        // Use the service current station or fall back to previous state
        final station = _audioPlayerService.currentStation ??
            (state is PlayerPlaying ? (state as PlayerPlaying).station : null) ??
            (state is PlayerPaused ? (state as PlayerPaused).station : null);

        if (station != null) {
          emit(PlayerPlaying(
            station: station,
            nowPlaying: _lastNowPlaying,
            volume: _audioPlayerService.volume,
            isFavorite: station.isFavorite,
          ));
        }
      } else {
        // Not playing: if we were playing, move to paused
  ...
  Future<void> _onPlayStation(PlayStation event, Emitter<PlayerState> emit) async {
    ...
    try {
      ...
      // Use the cached now-playing info if available (icy or AzuraCast)
      final now = _lastNowPlaying;
      emit(PlayerPlaying(
