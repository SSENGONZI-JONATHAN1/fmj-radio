...
class AudioPlayerService {
  ...
  Timer? _fmjPollingTimer;
  static const String _fmjApiUrl = 'http://165.245.232.129/api/nowplaying/fmj_radio';
  ...
  Future<void> _init() async {
    ...
    _audioPlayer.playerStateStream.listen((playerState) {
      // Ensure we only mark as playing after the player is ready and not buffering
      final isActuallyPlaying = playerState.playing && playerState.processingState == ProcessingState.ready;
      _isPlayingController.add(isActuallyPlaying);
    });
...
_audioPlayer.icyMetadataStream.listen((icyMetadata) {
      print('ICY metadata: $icyMetadata');
  ...
  Future<void> playStation(Station station, {int retryCount = 0}) async {
    try {
      ...
      // If the player reports ready and playing, update nowPlaying stream with latest info
      final playerState = _audioPlayer.playerState;
      final isActuallyPlaying = playerState.playing && playerState.processingState == ProcessingState.ready;
      if (isActuallyPlaying) {
        // If ICY metadata already provided a title, keep it; otherwise populate from AzuraCast polling
        if (nowPlayingInfo.title == 'Unknown') {
          // No ICY metadata; ensure polling or fallback will supply now-playing
          if (station.id == 'fmj_radio') {
            await _fetchFmjNowPlaying();
          }
        }
  ...
  Future<void> _fetchFmjNowPlaying() async {
    try {
      _log('Fetching FMJ now playing from $_fmjApiUrl...');
      final response = await http.get(Uri.parse(_fmjApiUrl)).timeout(const Duration(seconds: 10));
      
      if (response.statusCode == 200) {
        final responseBody = response.body;
        _log('FMJ API response: $responseBody');
        
        final data = json.decode(responseBody) as Map<String, dynamic>;
        final info = NowPlayingInfo.fromAzuraCast(data);
        
        _log('Parsed FMJ now playing: ${info.artist} - ${info.title}');
        _nowPlayingController.add(info);
      } else {
        _log('FMJ API error: HTTP ${response.statusCode}');
        _nowPlayingController.add(NowPlayingInfo(
  ...
  void _startFmjPolling() {
    _stopFmjPolling();
    _log('Starting FMJ now playing polling (10s interval)');
    _fmjPollingTimer = Timer.periodic(const Duration(seconds: 10), (_) => _fetchFmjNowPlaying());
    // Fetch immediately
    _fetchFmjNowPlaying();
  }


  void _stopFmjPolling() {
    _fmjPollingTimer?.cancel();
    _fmjPollingTimer = null;
  }
}
